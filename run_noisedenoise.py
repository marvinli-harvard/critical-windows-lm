...
python run_noisedenoise.py --model_id meta-llama/Llama-3.1-8B-Instruct \
    --dataset competition_math --split train --bs 4 --task math
"""
...
def main():
    ...
    if args.response_dataset is None:
        ...
        for batch in tqdm(dataloader):
            ...
            inputs, outputs, responses = generate_tokens(pipeline,question_tokens, max_gen_length=args.max_gen_length)
            ...
            for i in range(len(batch["problem"])):
                ...
                min_index = torch.where(outputs[i]==prompt_gen.heading_to_tokens["assistant"])[0][0]+2
                max_index = torch.where(outputs[i]==prompt_gen.heading_to_tokens["eot_id"])[0][-1]
                curr_value["response_tokens"] = outputs[i][min_index:max_index].cpu()
                curr_value["full_response_tokens"] = outputs[i].cpu()
                curr_value["full_response_string"] = responses[i]
    ...
    for gens in tqdm(first_responses):
        for copy, stop_frac in itertools.product(range(args.num_per_noise), args.percent_prompt):
            ...
            inputs = prompt_gen.get_noise_denoise_question(question=entry["problem"], 
                                                        response_tokens=entry["response_tokens"].tolist(),
                                                        stop_frac=stop_frac)
            entry["noised_denoised_input_tokens"] = list(inputs)
            entry["noised_denoised_input_string"] = pipeline.tokenizer.decode(inputs, skip_special_tokens=False)
            noised_questions_tokens.append(entry)
    
    ## Now run generation
    noised_denoised_results = []
    for batchno in tqdm(range(len(noised_questions_tokens) // args.bs)):
        ...
        inputs, outputs, responses = generate_tokens(pipeline,
                                          [b["noised_denoised_input_tokens"] for b in batch], 
                                          max_gen_length=args.max_gen_length)
        
        for i in range(len(batch)):
            curr_value = batch[i].copy()
            max_index = torch.where(outputs[i]==prompt_gen.heading_to_tokens["eot_id"])[0][-1]
            curr_value["full_response_tokens"] = outputs[i,:max_index].cpu()
            curr_value["full_response_string"] = responses[i]
