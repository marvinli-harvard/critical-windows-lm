...
def main():
    ...
    parser.add_argument('--story_str', action="store", type=str, required=False, default=DEFAULT_SYNTHETIC_STORY_PROMPT,help='Synthetic story')
    ...
    dataset = create_synthetic_form_dataset(genwrapper.tokenizer,template_str=args.story_str)
    ...
    ## Do some plotting
    ############################################################################################################################################################################################################################################
    synthetic_df = pd.DataFrame(generated_responses)
    synthetic_df["asst_response"]   = synthetic_df.response_string.apply(lambda x:extract_first_assistant_response(x))
    synthetic_df["choices"]         = synthetic_df.asst_response.apply(lambda x:tuple(extract_synthetic_choices(args.story_str,x)))
    synthetic_df["choices_tuple"]   = synthetic_df.choices.apply(lambda x:tuple([y[0] for y in x]))
    
    choices_df = synthetic_df[["asst_response","choices","choices_tuple"]].drop_duplicates()
    ...
    for i, final_str in tqdm(list(enumerate(choices_df.asst_response.unique()))):
        plot_critical_windows_given_str(percent_values, final_str, args.story_str,
                                        synthetic_df, args.step_size, 
                                        f"{args.experiment_dir}/ex_hierarchy/ex.{i}.png")

    synthetic_df.to_csv(f"{args.experiment_dir}/synthetic_responses.csv", index=False)
    synthetic_df.choices_tuple.value_counts().to_csv('synthetic_value_counts.csv', index=False)
    choices_df.to_csv(f"{args.experiment_dir}/synthetic_choices.csv", index=False)
...
def plot_critical_windows_given_str(percent_values : list[float],
    ...
    uniq_tuple = synthetic_df[synthetic_df.asst_response==final_str].choices_tuple.unique()[0]
