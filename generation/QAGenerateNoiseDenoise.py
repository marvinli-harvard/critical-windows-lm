...
class QAGenerateNoiseDenoise(GenerationBase):
    ...
    def generate_noised_denoised(self, percent_prompt : List[float]) -> List[Dict]:
    ...
    def generate_original_answer(self):
        ...
        self.orig_answers     = {} 
        outputs_orig, response_orig =  self.generate(orig_tokens, self.genwrapper.sampling_first) 
    ...
    def generate_stump_answers(self) -> List[Dict]:
        stump_responses = list(self.existing_stump_responses.items())    
        stump_tokens  = [item[1] for item in stump_responses]    
        self.stump_answers     = {} 
        outputs_stump, response_stump = self.generate(stump_tokens, self.genwrapper.sampling_first) 
        for i in range(len(stump_responses)):
            ...
            self.stump_answers[stump_responses[i][0]] = {
                ...
                "stump_ans_format": extract_answer(new_string, self.answer_type),
            }
        return self.stump_answers

    def generate_noised_denoised_answer(self) -> List[Dict]:
        self.results = []
        ...
        outputs_noise, response_noise = self.generate(noised_denoised_results_tokens, self.genwrapper.sampling_first) 
